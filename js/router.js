...
async function renderArticle(path) {
    ...
    document.querySelectorAll('.nav-item').forEach((n) => {
        n.classList.toggle('text-kvant', n.id === 'nav-' + store.lastPage);
    });

    const area = document.getElementById('article-content');
    area.innerHTML =
        '<div class="flex justify-center p-12 md:p-20"><i class="fas fa-circle-notch fa-spin text-3xl md:text-4xl text-kvant"></i></div>';
