...
function initEvents() {
    ...
    document
        ...
        ?.addEventListener('click', () => (window.location.hash = 'home'));
    document
        .getElementById('nav-tracks')
        ?.addEventListener('click', () => (window.location.hash = 'tracks'));
    document
        .getElementById('nav-cheats')
        ?.addEventListener('click', () => (window.location.hash = 'cheats'));
    document
        .getElementById('nav-projects')
        ?.addEventListener('click', () => (window.location.hash = 'projects'));
    document
        .getElementById('nav-games')
        ?.addEventListener('click', () => (window.location.hash = 'games'));
    ...
    document.getElementById('mobile-nav-tracks')?.addEventListener('click', () => {
        window.location.hash = 'tracks';
        toggleMobileMenu();
    });
    document.getElementById('mobile-nav-cheats')?.addEventListener('click', () => {
        window.location.hash = 'cheats';
        toggleMobileMenu();
    });
    document.getElementById('mobile-nav-projects')?.addEventListener('click', () => {
        window.location.hash = 'projects';
        toggleMobileMenu();
    });
    document.getElementById('mobile-nav-games')?.addEventListener('click', () => {
    ...
    // Главная страница
    document
        .getElementById('btn-show-all-tracks')
        ?.addEventListener('click', () => (window.location.hash = 'tracks'));
    document
        .getElementById('btn-show-all-projects')
        ?.addEventListener('click', () => (window.location.hash = 'projects'));
    document
        .getElementById('btn-portfolio-prev')
        ?.addEventListener('click', () => scrollPortfolio(-400));
    document
        .getElementById('btn-portfolio-next')
        ?.addEventListener('click', () => scrollPortfolio(400));

    // Просмотр статьи
    document.getElementById('btn-back-from-article')?.addEventListener('click', goBackSafe);
    document
        .getElementById('btn-article-history-back')
        ?.addEventListener('click', () => window.history.back());
    document
        .getElementById('btn-article-history-forward')
    ...
    document
        .getElementById('btn-graph-history-back')
        ?.addEventListener('click', () => window.history.back());
    document
        .getElementById('btn-graph-history-forward')
        ?.addEventListener('click', () => window.history.forward());
        ...
        if (btnRead) {
            ...
            import('./progress.js').then((m) => {
                ...
                import('./ui.js').then((ui) => {
                    const currentPath = window.location.hash.startsWith('#article:')
