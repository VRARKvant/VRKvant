    ...
    if (typeof marked !== 'undefined' && typeof hljs !== 'undefined') {
        const renderer = {
            image(token) {
                ...
                const href =
                    typeof token === 'object' && token !== null ? token.href : arguments[0];
                const title =
                    typeof token === 'object' && token !== null ? token.title : arguments[1];
                const text =
                    typeof token === 'object' && token !== null ? token.text : arguments[2];
...
export function processCustomTags(text) {
    ...
    text = text.replace(/\[gallery:\s*(.+?)\]/g, (match, imagesStr) => {
        ...
        images.forEach((img) => {
            html += `<div class="w-full shrink-0 flex items-center justify-center"><img src="${img}" class="max-w-full max-h-[60vh] md:max-h-[75vh] w-auto m-0 pointer-events-none" style="display:block;"></div>`;
        });
        html += `</div>`;
        if (images.length > 1) {
            ...
            images.forEach((_, i) => {
                html += `<div class="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full transition-colors duration-300 ${i === 0 ? 'bg-white' : 'bg-white/40'} shadow-md" id="${id}-dot-${i}"></div>`;
            });
            html += `</div>`;
        }
        html += `</div>`;
...
window.moveGallery = function (id, dir) {
    ...
    if (!window.galleryData[id])
        window.galleryData[id] = { index: 0, count: track.children.length };
