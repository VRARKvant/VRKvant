...
export function processCustomTags(text) {
    ...
    text = text.replace(/\[compare:\s*(.+?)\s*\|\s*(.+?)\]/g, (match, img1, img2) => {
        return `<div class="relative grid w-fit max-w-full mx-auto overflow-hidden my-6 shadow-xl rounded-xl select-none place-items-center"><img src="${img2.trim()}" class="col-start-1 row-start-1 max-w-full max-h-[60vh] md:max-h-[75vh] w-auto block m-0 pointer-events-none" alt="После"><img src="${img1.trim()}" class="compare-before col-start-1 row-start-1 max-w-full max-h-[60vh] md:max-h-[75vh] w-auto block m-0 pointer-events-none" style="clip-path: inset(0 50% 0 0);" alt="До"><input type="range" min="0" max="100" value="50" class="col-start-1 row-start-1 absolute inset-0 w-full h-full opacity-0 cursor-ew-resize z-20 m-0" oninput="window.updateCompare(this)"><div class="compare-handle absolute top-0 bottom-0 w-1 bg-white pointer-events-none z-10 left-1/2 -translate-x-1/2 shadow-[0_0_10px_rgba(0,0,0,0.5)]"><div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-white rounded-full flex items-center justify-center text-slate-800 shadow-md"><i class="fas fa-arrows-alt-h text-sm"></i></div></div></div>`;
    });
