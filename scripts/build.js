...
function generateManifests(articlesDir = 'articles/') {
            ...
            if (item === 'cheats') {
                ...
                files.forEach((file) => {
                    ...
                    projects.push({
                        ...
                        image: data.image || '',
                        file,
                        order: parseInt(data.order || 9999)
                    });
                    searchIndex.push({
                        title,
                        content: cleanText,
                ...
                files.forEach((file) => {
                    ...
                    lessonsData.push({
                        ...
                        file,
                        module: moduleName,
                        order: parseInt(data.order || (isIntro ? -1 : 9999))
                    });
                    ...
                    if (!isIntro) {
                        ...
                        if (!modulesInTrack[moduleKey]) {
                            modulesInTrack[moduleKey] = addNode(moduleName, moduleKey, 'module', {
                                url: articlePath
                            });
                            graph.links.push({
                                source: modulesInTrack[moduleKey],
                                target: trackNodeId
                            });
                        }
                ...
                const lessons = lessonsData.sort(
                    (a, b) => a.order - b.order || a.file.localeCompare(b.file)
                );
                let trackMetadata = {};
                ['intro.md', 'index.md'].forEach((introFile) => {
                    const introPath = path.join(itemPath, introFile);
                    if (fs.existsSync(introPath))
                        trackMetadata = parseMD(fs.readFileSync(introPath, 'utf-8')).data;
                });
                tracks.push({
                    id: trackId,
                    name: trackMetadata.name || trackName,
    ...
    console.log(
        'Манифесты (tracks, cheats, portfolio, games, gallery, search_index, graph) успешно сгенерированы.'
    );
}
